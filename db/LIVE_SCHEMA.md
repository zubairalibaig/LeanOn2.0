...
| Migration | What it does | Status |
|---|---|---|
| `061_push_tokens.sql` | New `push_tokens` table (token PK, user_id, user_agent, created_at, last_seen_at; RLS on, service-role only) — one row per device so a listener's phone AND laptop both get request alerts. Copies existing `users.fcm_token` values in. | **Run in Supabase SQL Editor.** Safe any time: until run, the code keeps using the single `users.fcm_token` column (last device registered wins). |
